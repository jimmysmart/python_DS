...
FROM Facilities
WHERE membercost>0
AND membercost < maintenance * 0.2
...
SELECT name, montlymaintenance
    CASE WHEN monthlymaintence > 100 THEN 'expensive'
...
SELECT f.name AS tennis_court_id, bookid, CONCAT( m.surname, ",", " ", m.firstname ) AS member_name
FROM `Bookings` b
...
SELECT f.name, CONCAT( m.surname, ",", " ", m.firstname ) AS member_name,
CASE WHEN m.surname LIKE 'Guest'
THEN f.guestcost
ELSE f.membercost
END AS cost
FROM Members m
